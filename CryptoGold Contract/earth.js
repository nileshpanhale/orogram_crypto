...
async function checkBal(account) {
    ...
    let bal = await contract.methods.balOf(account).call();
    bal = Math.trunc(bal);
    bal = bal / (10 ** 8);
    return bal;
 ...
 */
async function mint(receiver, mongoId) {
...
async function Transfer(sender, pvtKey, receiver, amt) {
    ...
    const transferFunction = contract.methods.transfer(receiver, tokenId).encodeABI();
    const NetworkId = await web3.eth.net.getId();
