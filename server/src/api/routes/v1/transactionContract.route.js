...
const controller = require('../../controllers/transactionContract.controller');
...
router
     ...
     * @apiError (Bad Request 400)  ValidationError  Some parameters may contain invalid values
     */
    .get(controller.getallContracts)
...
app.post('/uploadmultiple', upload.array('myFiles', 12), (req, res, next) => {
   ...
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .post(fileUploadConfig.fileUploadConfig.array("files"), controller.uploadFile);


  //search all user by Email id API here ----------------------------------------------

  router
    .route('/searchUser/:query')
    
    .get(authorize(), controller.getUserSearch)


  //search Description of contract by word API here ----------------------------------------------

  router
    .route('/searchDescription/:query')
    
    .get(authorize(), controller.getDescriptionSearch)

  // getting txhash Details --------------------------------------------------------------------
    router
    .route('/getContractDetails/:hash')
    .get(authorize(), controller.getContractDetails) 

    router
    .route('/getTradeDetail/:hash')
    .get(authorize(), controller.getTradeDetail) 

    router
    .route('/getAdminTxDetail/:hash')
    .get(authorize(), controller.getAdminTxDetail) 

    router.route('/purchaseLand')
    .post(controller.mintLandNFT);

    router.route('/mapRead')
    .get(controller.readMap);
