...
router
  ...
  /**
   * @api {put} v1/users/updatePassword  Update User's password
   ...
   * @apiHeader {String} Authorization  User's access token
   *
   * @apiParam  {String}   oldPassword        User's email
   * @apiParam  {String}   password     User's new password
   * @apiParam  {String}   confirmPassword  User's new password
   *
   *
   ...
   * @apiError (Unauthorized 401)  Unauthorized  Only authenticated Users can access the data
   */
  .put(authorize(), controller.updatePassword);
...
router
   ...
   */
  .put(authorize(), fileUploadConfig.single('profileImage'), controller.uploadFile);
   ...
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .put(authorize(), fileUploadConfig.single('doc'), controller.uploadDoc);

router
  .route('/googleAuth')
  /**
   * @api {put} v1/users/googleAuth/?status= Google auth
   * @apiDescription Enable/disable Google auth
   * @apiVersion 1.0.0
   * @apiName Google auth
   * @apiGroup User
   * @apiPermission user
   ...
   * @apiHeader {String} Athorization  User's access token
   *
   * @apiParam {Number}  status        1/0 to enable/disable Google auth (query param)
   *
   * @apiSuccess {String}            -         svg of qr code
   *
   * @apiError (Bad Request 400)  ValidationError  Some parameters may contain invalid values
   * @apiError (Unauthorized 401) Unauthorized Only authenticated users can modify the data
   * @apiError (Forbidden 403)    Forbidden    Only user with same id or admins can modify the data
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .put(authorize(), controller.googleAuth  )

router
  .route('/bankAccounts')
  /**
   * @api {put} v1/users/bankAccounts save bank accounts
   * @apiDescription Save bank accounts
   * @apiVersion 1.0.0
   * @apiName save bank accounts
   * @apiGroup User
   * @apiPermission user
   *
   * @apiHeader {String} Athorization  User's access token
   ...
   * @apiParam  {String}               country          Country of the bank
   * @apiParam  {String}               swiftCode        Swift code of the bank
   *
   ...
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .post(authorize(), controller.saveBankAccount  )

  router
    .route('/modifyAccount')
    /**
     * @api {put} v1/users/modifyAccount update account detail in user profile
     * @apiDescription Save bank accounts
     * @apiVersion 1.0.0
     * @apiName save bank accounts
     * @apiGroup User
     ...
     * @apiError (Not Found 404)    NotFound     User does not exist
     */
    .post(authorize(), controller.userBankAccount  )

router
  .route('/balance')
  /**
   * @api {get} v1/users/balance User Balane
   * @apiDescription Get logged in user balance information
   * @apiVersion 1.0.0
   * @apiName get balance
   * @apiGroup User
   * @apiPermission user
   ...
   * @apiError (Unauthorized 401)  Unauthorized  Only authenticated Users can access the data
   */
  .get(authorize(), controller.getBalance);

router
  .route('/account')
  /**
   * @api {get} v1/users/account User blockchain account
   ...
   * @apiError (Unauthorized 401)  Unauthorized  Only authenticated Users can access the data
   */
  .get(authorize(), controller.getAccount1);
...
router
   ...
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .patch(authorize(), controller.updateUser)
  /**
